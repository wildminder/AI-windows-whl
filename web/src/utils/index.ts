...
export function filterWheels(packages: Package[], options: FilterOptions): Package[] {
  ...
  return packages
    .map((pkg) => {
      ...
      if (pythonVersion) {
        filteredWheels = filteredWheels.filter(
          (w) => versionMatchesFilter(w.python_version, pythonVersion),
        );
      }
      
      if (torchVersion) {
        filteredWheels = filteredWheels.filter(
          (w) => versionMatchesFilter(w.torch_version, torchVersion),
        );
      }
      
      if (cudaVersion) {
        filteredWheels = filteredWheels.filter(
          (w) => versionMatchesFilter(w.cuda_version, cudaVersion),
        );
      }
...
export function versionMatchesFilter(wheelVersion: VersionRange | null | undefined, filterVersion: string): boolean {
